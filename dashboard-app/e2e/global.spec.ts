...
test.describe("Global Overview Page (Super Admin)", () => {
	...
	test("should load global overview page", async ({ page }) => {
		...
		const hasGlobalPage = await page.getByText(/Global Overview|Access denied/i).first().isVisible();
...
test.describe("Theme Switching", () => {
	test("should have theme toggle in sidebar", async ({ page }) => {
		...
		const themeToggle = page.locator("button").filter({ has: page.locator("svg") }).filter({ hasText: /sun|moon/i });
		...
		const hasThemeControl =
			(await themeToggle.first().isVisible().catch(() => false)) ||
			(await themeButton.first().isVisible().catch(() => false));
