...
function filterByTimeRange<T extends { createdAt?: Date; startedAt?: Date }>(
	items: T[],
	from: Date,
	to: Date,
): T[] {
...
function computeSessionMetrics(
	...
	const sessionHandoffs = handoffs.filter(
		(h) => h.sessionId === session.sessionId,
	);

	const lifespanMs =
		session.lastMessageAt.getTime() - session.firstMessageAt.getTime();
	const activeTimeMs = sessionRuns.reduce((sum, r) => sum + r.executionMs, 0);
	const successfulRuns = sessionRuns.filter(
	...
	const hasPostHandoffIteration =
		...
		sessionRuns.some((run) => {
			...
			return (
				"timestamp" in lastHandoff &&
				run.startedAt > (lastHandoff as { timestamp: Date }).timestamp
			);
		});
...
function computeUserMetrics(
	...
	const lifespans = userSessions.map(
		(s) => s.lastMessageAt.getTime() - s.firstMessageAt.getTime(),
	);
	...
	const sessionsWithHandoff = new Set(userHandoffs.map((h) => h.sessionId))
		.size;
	const localHandoffRate =
		sessionCount > 0 ? (sessionsWithHandoff / sessionCount) * 100 : 0;
	...
	const successfulRuns = userRuns.filter(
		(r) => r.status === "SUCCEEDED",
	).length;
...
export function getOrgMetrics(
	...
	const currentSuccess = currentRuns.filter(
		(r) => r.status === "SUCCEEDED",
	).length;
	const prevSuccess = prevRuns.filter((r) => r.status === "SUCCEEDED").length;
	const currentSuccessRate =
		currentRuns.length > 0 ? (currentSuccess / currentRuns.length) * 100 : 0;
	const prevSuccessRate =
		prevRuns.length > 0 ? (prevSuccess / prevRuns.length) * 100 : 0;
	...
	const p95DurationMs = computeTrendData(
		percentile(currentDurations, 95),
		percentile(prevDurations, 95),
	);
	...
	// Friction KPIs
	const currentAvgRuns =
		currentSessions.length > 0
			? currentRuns.length / currentSessions.length
			: 0;
	const prevAvgRuns =
		prevSessions.length > 0 ? prevRuns.length / prevSessions.length : 0;
	const avgRunsPerSession = computeTrendData(currentAvgRuns, prevAvgRuns);

	const currentActiveTimes = currentSessions.map((s) => {
		const sessionRuns = currentRuns.filter(
	...
	const avgActiveTimeMs = computeTrendData(
		average(currentActiveTimes),
		average(prevActiveTimes),
	);

	const currentLifespans = currentSessions.map(
		(s) => s.lastMessageAt.getTime() - s.firstMessageAt.getTime(),
	);
	const prevLifespans = prevSessions.map(
		(s) => s.lastMessageAt.getTime() - s.firstMessageAt.getTime(),
	);
	const avgLifespanMs = computeTrendData(
		average(currentLifespans),
		average(prevLifespans),
	);

	const currentSessionIds = new Set(currentSessions.map((s) => s.sessionId));
	const currentHandoffSessions = new Set(
		allHandoffs
			.filter((h) => currentSessionIds.has(h.sessionId))
			.map((h) => h.sessionId),
	).size;
	const currentHandoffRate =
		currentSessions.length > 0
			? (currentHandoffSessions / currentSessions.length) * 100
			: 0;

	const prevSessionIds = new Set(prevSessions.map((s) => s.sessionId));
	const prevHandoffSessions = new Set(
		allHandoffs
			.filter((h) => prevSessionIds.has(h.sessionId))
			.map((h) => h.sessionId),
	).size;
	const prevHandoffRate =
		prevSessions.length > 0
			? (prevHandoffSessions / prevSessions.length) * 100
			: 0;

	const localHandoffRate = computeTrendData(currentHandoffRate, prevHandoffRate);

	// Post-handoff iteration (simplified estimate)
	const postHandoffIterationRate = computeTrendData(
		currentHandoffRate * 0.3,
		prevHandoffRate * 0.3,
	);
...
export function getSessionsList(
	...
	let sessionsWithMetrics = orgData.map((d) =>
		computeSessionMetrics(d.session, allRuns, allHandoffs),
	);
	...
	if (filters.search) {
		...
		sessionsWithMetrics = sessionsWithMetrics.filter(
			(s) =>
				...
				s.createdByUser.email.toLowerCase().includes(search),
		);
	}
	...
	const summary = {
		...
		avgRunsPerSession:
			sessionsWithMetrics.length > 0
				? Math.round(
						(sessionsWithMetrics.reduce((sum, s) => sum + s.runCount, 0) /
							sessionsWithMetrics.length) *
							10,
					) / 10
				: 0,
		avgActiveTimeMs:
			sessionsWithMetrics.length > 0
				? Math.round(
						sessionsWithMetrics.reduce((sum, s) => sum + s.activeTimeMs, 0) /
							sessionsWithMetrics.length,
					)
				: 0,
		avgLifespanMs:
			sessionsWithMetrics.length > 0
				? Math.round(
						sessionsWithMetrics.reduce((sum, s) => sum + s.lifespanMs, 0) /
							sessionsWithMetrics.length,
					)
				: 0,
		handoffRate:
			sessionsWithMetrics.length > 0
				? Math.round(
						(sessionsWithMetrics.filter((s) => s.localHandoffCount > 0).length /
							sessionsWithMetrics.length) *
							100 *
							10,
					) / 10
	...
	const pageData = sessionsWithMetrics.slice(
		startIdx,
...
export function getUsersList(
	...
	const filteredHandoffs = allHandoffs.filter((h) =>
		filteredSessionIds.has(h.sessionId),
	);
	...
	const summary = {
		...
		avgHandoffRate:
			usersWithMetrics.length > 0
				? Math.round(
						(usersWithMetrics.reduce((sum, u) => sum + u.localHandoffRate, 0) /
	...
	const totalPages = Math.ceil(totalItems / pagination.pageSize);
	const startIdx = (pagination.page - 1) * pagination.pageSize;
	const pageData = usersWithMetrics.slice(
		startIdx,
		startIdx + pagination.pageSize,
	);
...
export function getUserDetail(
	...
	const avgDailyRuns = userSessions.length > 0 ? metrics.runCount! / 30 : 0;
...
export function getUserSessions(
	...
	return filteredSessions.map((session) =>
		computeSessionMetrics(session, allRuns, allHandoffs)
	);
}
...
export function getGlobalOrgs(
	...
	const orgsWithMetrics: OrgWithMetrics[] = orgs.map((org) => {
		...
		return {
			...
			avgRunsPerSession:
				sessions.length > 0 ? runs.length / sessions.length : 0,
			localHandoffRate:
				sessions.length > 0 ? (sessionsWithHandoff / sessions.length) * 100 : 0,
...
export function getOrgFailures(
	...
	for (const run of failedRuns) {
		if (run.failureCategory) {
			categoryCounts.set(
				run.failureCategory,
				(categoryCounts.get(run.failureCategory) || 0) + 1,
			);
		}
	}
